...
export default function ShopPage() {
  ...
  const [selectedTags, setSelectedTags] = useState<string[]>(
    searchParams.get("tags__name")
      ? [searchParams.get("tags__name") as string]
      : []
  );
  const [sortBy, setSortBy] = useState<string>(
    searchParams.get("ordering") || "featured"
  );
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      setLoading(true);
      try {
        // Convert sortBy to API ordering parameter
        let ordering = "";
        ...
        setTotalPages(Math.ceil(response.count / 10)); // Assuming 10 products per page
        setLoading(false);
      } catch (error) {
